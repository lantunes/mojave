...
public class HttpActionInvoker implements ActionInvoker {
	...
	private View invokeActionMethod( ) throws Exception {
		...
		List<Object> classInterceptors = createInterceptors( 
				controllerDb.getInterceptorsFor( actionControllerClass ) );
		...
		for ( Object interceptor : classInterceptors ) {
			...
			view = invokeBeforeActionIfRequired( interceptor, 
					controllerDb.getBeforeActionMethodForInterceptor( interceptor.getClass( ) ), 
					args );
			if ( view != null ) {
				return view;
			}
		...
		view = invokeBeforeActionIfRequired( actionController, 
				controllerDb.getBeforeActionMethodFor( actionControllerClass ), 
				args );
		...
		view = (View)actionFastClass.invoke( actionSignature.fastIndex( ), 
				actionController, args );
		logger.debug( "invoked " + actionSignature.methodName( ) + " for " + 
				actionControllerClass.getName( ) );
	...
	private List<Object> createInterceptorsForAction( ) throws Exception {
		...
		List<Class<?>> interceptorClasses = null;
			interceptorClasses = actionSignature.getInterceptorClasses( controllerDb, 
					actionControllerClass, action );
	...
	private View invokeBeforeActionIfRequired( Object instance, 
			ActionSignature actionMethod, Object[] args ) throws Exception {
		
		return invokeBeforeOrAfterActionIfRequired( instance, actionMethod, args, "before" );
	}
	
	private View invokeAfterActionIfRequired( Object instance, 
			ActionSignature actionMethod, Object[] args ) throws Exception {
		
		return invokeBeforeOrAfterActionIfRequired( instance, actionMethod, args, "after" );
	}
	
	private View invokeBeforeOrAfterActionIfRequired( Object instance, 
			ActionSignature actionMethod, Object[] args, String which ) throws Exception {
		...
		if (actionMethod != null) {
			...
			Object returnObj = actionFastClass.invoke( actionMethod.fastIndex( ), 
					instance, getBeforeOrAfterActionArgs(actionMethod.parameterTypes(), args) );
			...
			/*
			 * if there is a before or after action method, it is possible
			 * that another view may have been dispatched to from that
			 * method; therefore if the response has been committed, we
			 * should return an empty view
			 * NOTE: this will not work if the servlet container does not
			 * set the response to being committed after the dispatch 
			 */
			if (response.isCommitted( )) return new EmptyView( );
		}
	...
	private Object[] getBeforeOrAfterActionArgs( Class<?>[] paramterTypes, Object[] actionArgs ) {
		...
		if ( paramterTypes != null && paramterTypes.length == 1 && 
				paramterTypes[0].equals(RequestContext.class) ) {
