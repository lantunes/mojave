 ...
 * per &#064;Action method per controller. It is immutable once created,
 * and all of its public methods are thread-safe.
 * 
 * @author Luis Antunes
 */
public class BaseActionSignature implements ActionSignature {
	 ...
	 * 
	 * someSignature( @Param("p1") String p1, @Form SomeForm form, @Param("p2") Date p2 )
	 * 
	...
	public BaseActionSignature( int fastIndex, String methodName, 
	...
	private void populateArgsForForms( Class<?> formType, Map<String, ?> parametersMap, 
			List<PropertyDescriptor> params, List<Object> args ) {
		...
		try {
			...
			for ( PropertyDescriptor propertyDescriptor : params ) {
				...
				Class<?> setterParamType = setter.getParameterTypes( )[0]; /* there should be only one */
	...
	private boolean getBooleanValue( String val ) {
		...
		throw new IllegalArgumentException("boolean value not in correct " +
				"format('t','true','1';'f','false','0'): " + (val == null ? "null" : val));
	}
}