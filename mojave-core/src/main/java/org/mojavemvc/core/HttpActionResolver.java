...
public class HttpActionResolver implements ActionResolver {
	...
	public HttpActionResolver( ControllerContext context, HttpServletRequest req, 
			HttpMethod httpMethod, ControllerDatabase controllerDb, Injector injector ) {
	...
	private void resolveControllerClass( final String controller ) {
		...
		if (controller != null) 
		{
			...
			if ( controllerClass == null ) {
				throw new RuntimeException("no controller defined for controller " +
						"variable: " + controller);
			}
		}
		else 
		{
			...
			if (controllerClass == null) {
				throw new UnsupportedOperationException( "no controller variable was specified, " +
						"and no @"+ DefaultController.class.getSimpleName( ) + " exists" );
			}
		}
	...
	private void invokeAfterConstructIfRequired( ) throws Exception {
		...
		if (afterConstructSig != null) {
			...
			actionFastClass.invoke( afterConstructSig.fastIndex( ), 
					actionController, new Object[]{} );
	...
	private void resolveActionSignature( final String action ) {
		...
		if ( action == null || action.trim().length() == 0) {
			...
			actionSignature = controllerDb.getHttpMethodActionSignature( 
					controllerClass, httpMethod );
			...
			if ( actionSignature == null ) {
				...
				if ( actionSignature == null ) {
					throw new UnsupportedOperationException( "no default action found in " + 
			...
			actionSignature = 
				controllerDb.getActionMethodSignature( controllerClass, action );
			
			/*
			 * if the actionSignature is null, an invalid action
			 * parameter was given
			 */
			if ( actionSignature == null ) {
				throw new IllegalArgumentException( "no action '" + action + 
						"' defined for " + controllerClass );
			}
		}
	}
